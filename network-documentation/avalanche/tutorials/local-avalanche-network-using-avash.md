---
description: Learn how to setup up a local avalanche network using Avash
---
...
[Avash](https://github.com/ava-labs/avash) is a temporary statefull shell client which could be used for various purposes like deploying local avalanche networks, managing their processes and for running network tests. But, once we exit from avash, all these gets wiped off. This provides us with the opportunity to play around and experiment with the avalanche networks on our local system without really connecting to real networks on the internet. Avash also provides us with the ability to write scripts using lua which enables us to automate the creation of such networks and their configuration. In this tutorial, we're going to install a copy of avash on our machine, and create a lua script which could be used to fire up a 5 node staking network.
...
[AvalancheGo](https://github.com/ava-labs/avalanchego) is the official node implementation of the avalanche network. Avash needs an executable of this node implementation present for it's proper working. So, we need to install AvalancheGo before we try to run Avash.
...
Since go 1.16, module-aware mode is enabled by default and this along with many other things, means that when we execute "go get ...", the project gets downloaded to $GOPATH/pkg/mod and the permissions on this directory is very tight that we won't be able to execute scripts/build.sh for building avalanchego and so we turn this mode off for our installation of avalanchego. I hope to resolve this incompatibility between versions in the future, but for now we've to take care of this ourselves.
...
After the build process is complete, you can find the Avalanche binary, named avalanchego, in the build directory.

## Avash Installation

Now we go onto install avash. Unlike avalanche, avash needs module-aware mode enabled for it to be successfully installed.

```bash
go get github.com/ava-labs/avash
```

If this command fails to execute with similar errors like below, that means we've to turn on module-aware mode explicitly.
...
To continue, we use the same command like before but we pass in an extra environment variable to explicitly turn on module-aware mode:

```bash
GO111MODULE=on go get github.com/ava-labs/avash
```

Now we have the code for avash dowloaded onto our macines at this point. Again we have some differences in behaviour based on golang versions being used.
...
However, for those who're using go versions < 1.16, you have to manualy build the avash source:
...
Now that we have a successful installation of avash on our machine, we go ahead and add a lua script that we'll use to fire up avalanche network.

#### For go versions < 1.16:

We need to add a configuration file and a lua script to the scripts directory inside avash installation.
...
Now you need to run the script we created in the last section which will start a five node staking network on your machine.
...
When you're all done experimenting with the local avalanche network we just fired up, to exit from avash, type the following command in the avash propmt:

```bash
exit
```

This closes avash terminal and with it, all the nodes started during it's lifetime, essentially destroying the temporary local avalanche avalanche network we fired up using the lua script.

## Conclusion

In this tutorial, we've successfully managed to install avash, create a lua script that fires up a five node staking network on your machine and to actually fire it up and interact with the network from the terminal.
