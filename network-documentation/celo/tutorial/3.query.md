...
## Introduction
Now we have our account created, but wouldn’t it be nice to keep track of cUSD and CELO balances in our NodeJS app?
In this tutorial we will examine how we can do just that, plus some additional queries to get information about node and blocks.
...
Let’s get to it. In order to query account information we need to use `getTotalBalance` method which is exposed by ContractKit.

Create `query.js` file and in it paste in below snippet:
...
Once we create a new client using `newKit` we need to initialise our account. Luckily, our `.env` file holds all the information we need to do just that. This line:

```javascript
const account = web3.eth.accounts.privateKeyToAccount(process.env.PRIVATE_KEY);
```

uses private key from `.env` file and initializes account from it. 
Having account object to our disposal let’s run our first query:

```javascript
const accountBalances = await client.getTotalBalance(account.address)
```

This query uses `address` property from account object. There is one more interesting thing to notice. This time we do not use `web3.eth` to call `client.getTotalBalance` but instead we call it directly from the `client` object. What is going on here?  ContractKit is a wrapper for web3 client, which exposes some additional Celo specific methods. `client.getTotalBalance` is one of those methods.
...
* **Pending balance** - amount of CELO that is pending. When you vote for validator groups, after you cast your vote you have to wait until next epoch to activate you vote. If you don’t do that, your vote will be in pending bucket.

Now you can keep track of your Celo balances from your own application. Awesome!

## Querying node information
It is always a good idea to know what node version you are working with. Let’s examine how we can do that using ContractKit and DataHub Celo node.

Let’s add below snippet right after our last piece of code (1. Query account balances):
...
Here we use again `eth` client calling `eth.getNodeInfo`. 
...
It tells us that node exposed by DataHub is running version `v1.2.1-stable`  on the `linux-amd64` machine where Golang version `1.13.15` is installed.

## Querying block information
Blocks are the basic building blocks of every blockchain. It would be nice to get more information about them. `eth` client from `@celo/contractkit` npm package allows us to query details about blocks at certain height.

Let’s add below snipped right after our last piece of code (2. Query node info)
...
Here we run one query `getBlock` with two different arguments.
When we pass to this method `latest`  as an argument we query for the most recent block. If we pass a number, we query for block at that specific height. 
...
Here you can find information about timestamp at which this block has been created, transactions appended to this block and encrypted extraData which holds information about validators who signed previous block.

## Conclusion
Now you should have a basic understanding on how to query some of the most important information from the Celo node hosted by DataHub. In this tutorial we’ve learnt how to get more information about your account’s balances, node information and 

## Next steps
Querying for useful information if fun, but being able to submit transactions and change the state of blockchain is even better. In our next tutorial we will dive deeper and submit our first transactions.
