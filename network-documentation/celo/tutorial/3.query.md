# 3. Query the Celo blockchain
## Introduction
Now we have our account created, but wouldn’t it be nice to keep track of cUSD and CELO balances in our NodeJS app?
In this tutorial we will examine how we can do just that, plus some additional queries to get information about nodes and blocks.
...
When we pass `latest`  as an argument, we query for the most recent block. If we pass a number, we query for a block at that specific height. 
...
## Conclusion
Now you should have a basic understanding on how to query some of the most important information from the Celo node hosted by DataHub. In this tutorial we learned how to get more information about your account’s balances, node information and block information. 

## Next steps
Querying for useful information is fun, but being able to submit transactions and change the state of blockchain is even better. In our next tutorial we will dive deeper and submit our first transactions on Celo.
