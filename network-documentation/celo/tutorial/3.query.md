...
Here we run one query `getBlock` with two different arguments (Steps 3.1 and 3.2).
...
In step 4 we call `getTransactionCount` to get the number of transactions submitted so far for our account.
In step 5 we call `estimateGas` which generates and returns an estimate of how much gas is necessary to allow the transaction to complete. We pass in optional `data` parameter which is a hash of the method signature and encoded parameters.
