...
At the top of the file, the artifacts.require() line is what fetches the contract we'll be deploying. The name should always match the Solidity contract definition. In our migrations contract, the contract definition looks like: 
...
Next, open the .env file you created in your code editor and add the following variable as a new line, where **<YOUR_API_KEY>** is the API key from DataHub:
...
For our purposes, we will need to replace the existing code with the following following in your ``truffle-config.js`` file: 
...
It then connects to our Celo node by getting the REST_URL from .env: 

``const web3 = new Web3(process.env.REST_URL);`` 

Next it gets your account from the private key in the ``.env`` file in order to deploy from your Celo account: 
...
Now that you've finished the tutorial, you should have a basic understanding of deploying smart contracts on the Celo network. The possibilities are endless for what you can create!  You can use this tutorial as a jumping off point for deploying the smart contracts of your dreams   🥳

The complete source code for this tutorial can be found on [Github](https://github.com/alexreyes/Celo-Deploying-With-Truffle).

# Next steps

Now that you've learned how to deploy smart contracts, you can build new use cases for the cryptoeconomy on Celo. Feel free to learn more Solidity, or continue the tutorials on [Figment Learn](https://learn.figment.io).
