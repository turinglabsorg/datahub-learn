...
* A `ReferenceError: Tezos is not defined` would indicate an issue with the TezosToolkit. Check the lines where the Toolkit is imported, and where it is instantiated : `const Tezos = new TezosToolkit(process.env.DATAHUB_URL)`
{% endhint %}
