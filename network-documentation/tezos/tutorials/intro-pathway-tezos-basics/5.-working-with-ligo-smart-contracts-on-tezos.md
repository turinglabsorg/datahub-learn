...
{% tab title="REQUIRE\(\) SYNTAX /tezospath/counter.js" %}
```javascript
...
* **`SyntaxError: Invalid or unexpected token`** this indicates we need to check the value  entered into `Tezos.contract.at().` The value needs to be 'single quoted'. Supplying an address _without_ quotes will log an empty error object : **`Error: {}`** when run.
{% endhint %}
