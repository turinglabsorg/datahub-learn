...
* _If we are choosing to use the_ `import` _syntax_, we will need to add a line to our `package.json` to prevent a`SyntaxError: Cannot use import statement outside a module` . This is due to the `import` keyword \(another option would be to use an `.mjs` file extension for all the files, however adding this line to  will allow us to keep the `.js` file extension\) : 

{% tabs %}
{% tab title="Type or paste this line into package.json" %}
```javascript
"type": "module",
```
{% endtab %}
...
* Alternatively, if we wish to use the more common `require()` syntax, the line above is unnecessary. The relevant code blocks throughout the tutorial will have an additional tab which illustrates the `require()` syntax.

Now that this is complete, we are set up and ready to make our connection to Tezos.
...
Create a new file called `connect.js` and paste the following code :
