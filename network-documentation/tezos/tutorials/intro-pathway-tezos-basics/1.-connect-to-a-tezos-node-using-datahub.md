...
**ES6 syntax**  
For any and all errors related to the ES6 import syntax; make sure to follow the setup instructions carefully, also check to make sure that Node.js v12.22.1+ is installed. Do not use the `require()` syntax if `"type": "module"` has been specified in `package.json`.
...
**400 - Bad Request**  
The value supplied to getBalance\(\) is missing. The sample Tezos error message below says that the _the value "Cannot parse contract id" is not acceptable for type contract\_id_. The heart of this error is in the request URL. Where it says undefined it means that there was no value passed as an argument to `getBalance()`
