...
For maximum developer happiness, choose _**ONE**_  and use it throughout the tutorial or there will be errors. 
...
* If the value supplied to `getBalance()`is missing,  a`400` - `Bad Request` error indicates that the value cannot be parsed.
  ...
  The value we are referencing is the `BAKER_ADDRESS` , therefore we must ensure that the proper environment variable exists in `.env` 
{% endhint %}
