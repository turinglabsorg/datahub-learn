...
2. [Creating your first NEAR account with the NEAR Javascript API](https://learn.figment.io/network-documentation/near/tutorials/2.-creating-your-first-near-account-using-the-sdk)
...
Let's break this down a bit. At the top, as usual, we import NEAR Javascript API and `dotenv`. Everything looks the same but we have an extra package in use this time: `js-sha256`. We'll be using it to produce SHA256 hashes of our transactions.
...
Great job! Now you not only know how to read from the NEAR blockchain, but also how to interact with it by submitting transactions with NEAR Javascript API and DataHub.
