...
This might look familiar to you. Starting from the top you can see that we import Near SDK, dotenv, and then create a new client instance using `near.connect`. This establishes the connection to our NEAR node.
...
Finally, we're going to fetch the current gas price. While the other examples used the built-in SDK methods, this one will be using a direct RPC function call since the function to fetch gas price is not part of the SDK.
...
In the example above we've used a direct RPC call to fetch the information we need. While the SDK provides the majority of functions that you might need for basic operations \(like block fetching, transaction issuing, balance checks, etc\) it is important to know your options.
...
Congratulations! Now you know how to query a NEAR node with DataHub to get the information you need. For more queries please refer to the [NEAR API](https://learn.datahub.figment.io/network-documentation/near/rpc-and-rest-api/near-rpc) and [NEAR SDK](https://near.github.io/near-api-js/) documentation.
