...
Every type of NFT needs to define some metadata, to describe the individual interesting things being tracked: the name of a CryptoKitty, the title and author of an artwork, the date of a virtual ticketed event, or whatever else the NFT represents. In fact, most NFTs are mostly metadata, and the metadata usually refer to an image, video, or gif which is stored off-chain, say on a cloud storage service(like AWS S3) or preferably on a decentralized file storage service(like IPFS, Sia). 
