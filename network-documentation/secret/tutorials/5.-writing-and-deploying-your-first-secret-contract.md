...
```
----------------------------------------------------------------
Copy `transactionHash` from the output of either the contract upload transaction, or the contract execution to update the counter, and replace `<TRANSACTION HASH>` in the link below.
